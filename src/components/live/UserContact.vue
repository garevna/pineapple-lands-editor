<template>
  <v-card class="user-info mx-auto my-auto pa-6 homefone" width="480">
    ...
    <UserFormCustomization :dialog.sync="dialog" :pageNum="pageNum" />
  </v-card>
</template>
...
export default {
  ...
  computed: {
    ...
    userForm () {
      return this.pages[this.pageNum].userForm
    },
    title: {
      get () {
      ...
      set (val) {
        this.$store.commit('content/UPDATE_LIVE_PAGE_USER_FORM', {
          prop: 'title',
          value: val
        })
      }
  ...
  methods: {
    ...
    getFields () {
      return this.$store.state.contact.contactFormFields
    },
    initFields () {
